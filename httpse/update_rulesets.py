            ...
            for elem in root.iter():
                if elem.tag == 'target':
                    for domain in domains:
                    		if domain.split("//")[1] in elem.attrib['host']:
                    				rulesets[domain] = filename
        
        # remove "mixedcontent" tag from all rulesets
        for ruleset in glob.glob("*.xml"):
        		content = open(ruleset, 'r').readlines()
        		writeout = ''
        		for line in content:
        				if"<ruleset" in line:
        						if 'platform="mixedcontent"' in line:
        								line = re.sub(r'<ruleset(.*) platform="mixedcontent"(.*)>', r'<ruleset\1\2>', line)
        						elif "platform=" in line and " mixedcontent" in line:
        								line = re.sub(r'<ruleset(.*) platform="(.*) mixedcontent(.*)"(.*)>', r'<ruleset\1 platform="\2\3"\4>', line)
        						elif "platform=" in line and "mixedcontent " in line:
        								line = re.sub(r'<ruleset(.*) platform="(.*)mixedcontent (.*)"(.*)>', r'<ruleset\1 platform="\2\3"\4>', line)
        				writeout += line
        		open(filename,'w').write(writeout)
